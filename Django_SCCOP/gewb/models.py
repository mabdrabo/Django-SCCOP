...
class User(models.Model):
    ...
    def get_log(self):
        return Emergency.objects.filter(user=self).order_by('-create_date')

    def get_trackers(self):
        return [f.user1 if not f.user1==self else f.user2 for f in Track.objects.filter(Q(user1=self) | Q(user2=self))]
